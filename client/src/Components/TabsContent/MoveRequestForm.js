...
import {GetAllKindergardens, NewMoveRequest} from '../../Actions/NavActions/MoveRequestActions';
...
class MoveRequestForm extends Component {
    ...
    handleSubmitData = (event) => {
        ...
        if(form.ParentName === "")
        {
            this.setState({parentNameErrorMessage: "Obavezno uneti ime roditelja!"})
        ...
        {
            this.setState({childBirthErrorMessage: "Obavezno uneti datum rođenja deteta!"})
            this.setState({validationErrorExists: true})
        }
        if(form.FromKindergardenId === null)
        {
            this.setState({fromKindergardenErrorMessage: "Obavezno mesto relokacije!"})
            this.setState({validationErrorExists: true})
        }
            
        if(form.ToKindergardenIds.length === 0)
        {
            this.setState({toKindergardenErrorMessage: "Obavezno odabrati bar jednu želju za premestaj!"})
            this.setState({validationErrorExists: true})
        }
    ...
    checkIfHasErrors = () => {
        if(!(this.state.childNameErrorMessage ||
    ...
    render() {
        ...
        const { ParentNameSurname, ChildName, Email, PhoneNumber, ChildBirthDate, MoveFromLocationId,MoveToLocationId1,MoveToLocationId2,MoveToLocationId3 } = this.state;
            ...
            <form>
                <div className="form-wrap">
                    <div className="form-left">
                        <input type="text" id="newRequestParentName" className="input parent-input" placeholder="Ime i prezime roditelja *" onChange={this.handleParentNameChange} value={ParentNameSurname}/>
                        <span style={errorMsgStyle}>{this.state.parentNameErrorMessage}</span>
                        <input type="text" id="newRequestChildName" className="input child-input" placeholder="Ime deteta *" onChange={this.handleChildNameChange} value={ChildName}/>
                        <span style={errorMsgStyle}>{this.state.childNameErrorMessage}</span>
                        <input type="email" id="newRequestEmail" className="input input__email" placeholder="Email *" onChange={this.handleEmailChange} value={Email}/>
                        <span style={errorMsgStyle}>{this.state.emailErrorMessage}</span>
                        <div className="form-left__double">
                            <div>
                                <div>
                                    <div>
                                        <input type="text" id="newRequestPhoneNumber" className="input input__phone" placeholder="Broj telefona *" onChange={this.handlePhoneNumberChange} value={PhoneNumber}/>
                                    </div>
                                    <div>
                                        <span style={errorMsgStyle}>{this.state.phoneErrorMessage}</span>
                                    </div>
                                </div>

                                <div>
                                    <div>
                                        <input type="date" id="newRequestChildBirthDate" className="input input__date" placeholder="Datum rodjenja deteta" onChange={this.handleChildBirthDateChange} value={ChildBirthDate}/>
                                    </div>
                                    <div>
                                        <span style={errorMsgStyle}>{this.state.childBirthErrorMessage}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="form-right">
                        <div className="select-wrap">
                            <select id="newRequestFromKindergardenSelect" onChange={this.handleMoveFromLocationIdChange} value={MoveFromLocationId}>
                                <option value="" selected disabled hidden>Izaberi trenutnu lokaciju</option>
                                {
                            ...
                            <span style={errorMsgStyle}>{this.state.fromKindergardenErrorMessage}</span>
                        </div>
                        <div className="select-wrap">
                            <select id="newRequestToKindergarden1Select" onChange={this.handleMoveToLocationId1Change} value={this.props.prePopulatedId!==null?this.props.prePopulatedId : MoveToLocationId1}>
                                <option value="" selected disabled hidden>1. Lokacija na koju zelis da se premestis?</option>
                                {
                                    this.props.allKindergardens.map( kinder => (
                                        <option value={kinder.id}>{kinder.name}</option>
                                    ))
                        ...
                        <div className="select-wrap">
                            <select id="newRequestToKindergarden2Select" onChange={this.handleMoveToLocationId2Change} value={MoveToLocationId2}>
                                <option value="" selected disabled hidden>2. Lokacija na koju zelis da se premestis?</option>
                                {
                                    this.props.allKindergardens.map( kinder => (
                                        <option value={kinder.id}>{kinder.name}</option>
                                    ))
                        ...
                        <div className="select-wrap">
                            <select id="newRequestToKindergarden3Select" onChange={this.handleMoveToLocationId3Change} value={MoveToLocationId3}>
                                <option value="" selected disabled hidden>3. Lokacija na koju zelis da se premestis?</option>
                                {
                                    this.props.allKindergardens.map( kinder => (
                                        <option value={kinder.id}>{kinder.name}</option>
                                    ))
                            ...
                            <span style={errorMsgStyle}>{this.state.toKindergardenErrorMessage}</span>
                        </div>
                    </div>
                        ...
                        <ul className="form-social-links">
                            <li>
                                <a className="form-link" href="javascript:void(0);">
                                    <span className="font-ico-messenger"></span> Pisite nam </a>
                            </li>
                            <li>
                                <a className="form-link" href="javascript:void(0);">
                                    <span className="font-ico-facebook"></span> Posetite nas </a>
                            </li>
                        </ul>
                    </div>

                    <div className="form-buttons__right">
                        <button className="btn" type="submit" id="submit-new-request" onClick={this.handleSubmitData}>
                            <span className="font-ico-envelope"></span> Obavesti me </button>
                    </div>
...
const mapDispatchToProps = (dispatch) => {
    ...
    return {
        ...
        getAllKindergardens: () => {  dispatch(GetAllKindergardens()) },
        newMoveRequest: (formRequest) => {dispatch(NewMoveRequest(formRequest)) }
