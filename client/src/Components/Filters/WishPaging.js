    ...
    for (let i = 1; i <= this.props.pagesCount; i++) {
      pageNumbers.push(
        <PageNumber
          ...
          pageNumber={i}
          filterByPageHandler={this.filterByPage}
        />
      );
    }
      ...
      <div className="pagination">
        <div className="pagination__pages">
          {this.props.currentPage > 1 ? (
            <div className="pagination-directions__left">
              <a
...
const mapDispatchToProps = dispatch => {
  return {
    getAllWishes: filterByPage => {
      dispatch(GetAllWishes(null, filterByPage));
    },
    selectPageNumber: pageNumber => {
      dispatch(PageNumberSelector(pageNumber));
    },
    goNext: () => {
